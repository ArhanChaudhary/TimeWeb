...
function pset(ctx, x2 = false, y2 = false) {
    ...
    */
    ...
    if (ctx.a <= 0 || ctx.b > 0) {
        ...
        var funct_zero = precisionRound(-ctx.b / ctx.a, 10)
    }
    if (ctx.a >= 0) {
        var funct_y = x1;
    } else {
        var funct_y = precisionRound((Math.sqrt(ctx.b * ctx.b + 4 * ctx.a * y1) - ctx.b) / ctx.a / 2, 10);
    }
    if (ctx.funct_round < ctx.min_work_time) {
        ctx.cutoff_transition_value = 0;
        if (ctx.a) {
            ctx.cutoff_to_use_round = precisionRound((ctx.min_work_time_funct_round - ctx.b) / ctx.a / 2, 10) - 1e-10;
    ...
    if (y_value_to_cutoff > 0 && ctx.y > ctx.red_line_start_y && (ctx.a || ctx.b)) {
        ...
        ctx.return_y_cutoff = precisionRound(ctx.return_y_cutoff, 10);
    } else {
    ...
    if (y_value_to_cutoff < y1 && ctx.y > ctx.red_line_start_y && (ctx.a || ctx.b)) {
        ...
        ctx.return_0_cutoff = precisionRound(ctx.return_0_cutoff, 10);
    } else {
